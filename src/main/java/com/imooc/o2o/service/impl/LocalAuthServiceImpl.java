...
public class LocalAuthServiceImpl implements LocalAuthService{
	...
	public LocalAuthExecution bindLocalAuth(LocalAuth localAuth) throws LocalAuthOperationException {
		...
		try {
			...
			// 判断创建是否成功
			if (effectedNum <= 0) {
				throw new LocalAuthOperationException("bind fail");
			} else {
	...
	public LocalAuthExecution modifyLocalAuth(Long userId, String userName, String password, String newPassword)
		...
		if (userId != null && userName != null && password != null && newPassword != null
			...
			try {
				...
				// 判断更新是否成功
				if (effectedNum <= 0) {
					throw new LocalAuthOperationException("update fail");
				}
				return new LocalAuthExecution(LocalAuthStateEnum.SUCCESS);
			} catch (Exception e) {
				throw new LocalAuthOperationException("update fail:" + e.toString());
			}
		} else {
			return new LocalAuthExecution(LocalAuthStateEnum.NULL_AUTH_INFO);
		}
